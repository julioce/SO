...
int main(void){
	...
	while( m!= 0 && k!=0 ){
		...
		*compartilhado.variancia = 0;
		...
		fflush(stdout);
		//Neste programa estamos calculando o tempo necessário para realizar o produto interno utilizando subprocessos.
		//Começa a contagem do tempo de execução aqui!!
		ftime(&inicio_execucao);
		id = fork();
		if( id == 0){
			for(i=0; i<ceil(m/2); i++){
				...
				//esse if está redundante e errado!
				//if(id == 0){				
					for(j=0; j<k; j++){
		...
		//o fim do tempo de execução acontece aqui!
		ftime(&fim_execucao);
		printf("Concluído!\n\n");
